...
global
    ...
    // Nombres de archivos para que se incluyan obligatoriamente en la instalacion
    ...
    config[3]=0,1,0,1;  // Opciones de configuracion
    ...
    struct partida[7]   // Partidas guardadas
        ...
        balas;          // Municion de ametralladora
        granadas;       // Granadas
        bombas;         // Numero de explosivos
        tiros;          // Tiros a distancia para el rifle
        total[3];       // Municion de cada tipo utilizada
        string texto;
    end
    ...
    struct nivel[16]    // Dejar 16 (num. misiones mas 1)
    ...
    max_arma[]=250,20,10,10; // Municion maxima para cada arma

    total_max[]=98,18,8,8; // Frecuencia de utilizacion maxima de cada arma
...
begin
    ...
    // Sonido inicial de la explosion
    ...
    objeto(4,0,600,137,6); // Barra municion
...
// -------------------------------------------------------------------------
// Funcion principal de control de juego
// -------------------------------------------------------------------------
...
begin
    ...
    for (x=0;x<nivel[inicio.nivel].numero_1;x++)
        enemigo(3,nivel[inicio.nivel].tiros_1,nivel[inicio.nivel].energ¡a_1,280);
    end

    for (x=0;x<nivel[inicio.nivel].numero_2;x++)
        enemigo(4,nivel[inicio.nivel].tiros_2,nivel[inicio.nivel].energ¡a_2,280);
    end

    mas_enemigo_1=nivel[inicio.nivel].mas_1;
    mas_enemigo_2=nivel[inicio.nivel].mas_2;
...
process enemigo(file,municion,energia,max_dist)
...
private
    ...
    anim=0;
              ...
              // 0    - Parado, solo ve al prota si esta mirandole y en
              ...
              //        advirtiendo a los que tienen municion
...
begin
    ...
    loop
        ...
        if (energia<=0)
            ...
            // Soltar municion para el jugador
        ...
        // Accion en funcion del estado actual

        switch(estado)

            // *** Parado, en observacion
...
// -------------------------------------------------------------------------
// Marcador de la posicion destino del protagonista
// -------------------------------------------------------------------------
...
begin
    ...
    from angle=0 to 360000 step 20000;
        explosion(x+get_distx(angle,96+16),y+get_disty(angle,64));
    end
...
// -------------------------------------------------------------------------
// Gestion de cada uno de los cartuchos explosivos
// -------------------------------------------------------------------------
...
// -------------------------------------------------------------------------
// Gestion de las balas del protagonista
// -------------------------------------------------------------------------
...
// -------------------------------------------------------------------------
// Gestion de tiro en snipper mode (rifle)
// -------------------------------------------------------------------------
...
// -------------------------------------------------------------------------
// Gestion de la opcion de informes
// -------------------------------------------------------------------------

function informe(numero)

begin
    fade(180,0,0,10);
    while (fading) frame; end
    delete_text(all_text);
    load_screen("map\tokenkai\inf"+itoa(numero)+".pcx");
    fade(100,100,100,5);
    while (ascii!=0 or mouse.left) frame; end
end

// -------------------------------------------------------------------------
// Descarga los fuentes y ficheros graficos utilizados en los menus
// -------------------------------------------------------------------------
...
// -------------------------------------------------------------------------
// Gestion de la cargar de partidas grabadas
// -------------------------------------------------------------------------
...
// -------------------------------------------------------------------------
// Gestion de la grabacion de partidas
// -------------------------------------------------------------------------
...
// -------------------------------------------------------------------------
// Funcion que comprueba si el cursor del raton esta dentro de una zona
// -------------------------------------------------------------------------
...
// -------------------------------------------------------------------------
// Gestion del radar en el marcador
// -------------------------------------------------------------------------
...
// -------------------------------------------------------------------------
// Objetos graficos que se situan en pantalla
// -------------------------------------------------------------------------
...
begin
    file=1; graph=13;
    z=-1;
    write_int(1,600,117,4,offset municion);
    loop
        x=542+(arma_sel%2)*44;
        y=197+(arma_sel/2)*44;
        switch(arma_sel)
            case 0: municion=inicio.balas; end
            case 1: municion=inicio.granadas; end
            case 2: municion=inicio.tiros; end
            case 3: municion=inicio.bombas; end
        end
        define_region(6,579,131,(municion*42)/max_arma[arma_sel],13);
...
// -------------------------------------------------------------------------
// Representacion de cada enemigo como punto en el radar
// -------------------------------------------------------------------------
...
begin
    ...
    loop
        ...
        write_int(2,512,122+34*2,0,offset e_municion);
...
begin
    ...
    loop
        if (estado==1)
            from conta=0 to 6;
                explosion(x+rand(-32,32),y+rand(-32,32));
                sound(sonido[17],256,256);
            end
            estado=2;
            graph++;
